...
ϕ = .5
tk = 0.35 #taxes on capital
tc = 0.1800 #taxes on consumption
κ = .728 #initial guess
τ = 0.23 #be careful, it is 1-τ in Heathcote and κ is λ !
Tr_init = 0.0 # should try to reach 8%

#############Variables in the steady-state
KsY = 3.1*4 #capital to output ratio
...
(γ ==1.0) ? fu(c)=log.(c) : fu(c)=(c.^(1-γ).-1)./(1-γ)
(γ ==1.0) ? fuinv(X) = exp.(X) :  fuinv(X) = (1. +(1-γ).* ( X ) ).^(1/(1-γ))
fup(c) = c.^-γ
fupp(c) = -γ*c.^(-γ-1)
