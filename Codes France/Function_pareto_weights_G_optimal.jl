...
#######################
# Set negative weights to a minimum as a constraint
#######################
...
f(x...) = sum(disty[i]*(x[i]-1)^2 for i in 1:length(x))

model = Model();
vector_model = Model(Ipopt.Optimizer)

@variable(vector_model, x[1:ns]>=0.01)

f(x...) = sum(disty[i]*(x[i]-1)^2  for i in 1:length(x))
register(vector_model, :f, ns, f; autodiff = true)
...
weight2 = 1 ./(M7'*(Sp.*uc)./disty) #pareto weights with full instrments
...
return res,CC, J1, J2, J3, J4, J5, M7
end
