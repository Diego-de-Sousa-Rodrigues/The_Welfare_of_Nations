...
struct Xsis{T <: Real}
    xsiu0::Array{T,1} #correcting parameter for utility in level
    xsiu1::Array{T,1} #correcting parameter for u'
    xsiuE::Array{T,1} #correcting parameter for Euler equation
    xsiv0::Array{T,1} #correcting parameter for disutility of labor
    xsiv1::Array{T,1} #correcting parameter for v'
    xsiy::Array{T,1} #correcting parameter for labor
    xsil::Array{T,1} #correcting parameter for v'
end
...
include("parameters_US_final.jl") #file with the main parameters to obtain the steady state
...
x = cumsum(disty*100)
psih = M7'*(planner.psi + proj.Sp*planner.mu )./disty
y = psih 
ycum = cumsum(disty.*y)
itp_g = interpolate((x,), ycum, Gridded(Linear()))
ext_itp_g = extrapolate(itp_g, Line())
IOWpsid =zeros(Float64,5)
for i=1:5
    IOWpsid[i]=ext_itp_g(i*20)-ext_itp_g((i-1)*20)
end
#IOWpsid=IOWpsid/mean(IOWpsid)
plot(IOWpsid)
...
for i=1:5
    IOWd[i]=ext_itp_g(i*20)-ext_itp_g((i-1)*20)
end
IOW50 = ext_itp_g(50)
IOWd=IOWd/mean(IOWd)
...
## Plotting GSMWW
plot(IOWd,line = (3,:solid, [:blue]), label = "Weights", legend=:false)
savefig("GSMWW_USA.png")


Wage2008 = 51726 # in 2008$, average family income
x_new = Modeling.states*Wage2008
x = cumsum(disty*100)
uc = 1.0./weights_marginal # weights_marginal are construct as 1/uc
y = weights_param.*uc
yn = y/y[5]
...
df = CSV.read("data_US.csv", DataFrame)
x = Float64.(df[:,1])
y = df[:,2]
Wage2008 = 51726 #in 2008$, average family income
x_new = Modeling.states*Wage2008
...
function run_computation_and_save(parameters_file, suffix)
    ...
    sol = nlsolve(f!, [-1.; -1.; -1.], autodiff = :forward)
    a, b, c = sol.zero
...
run_computation_and_save("parameters_US_final_beta.jl", "beta") #US parameters with French beta
run_computation_and_save("parameters_US_final_beta_taxes.jl", "beta_taxes") #US parameters with French beta and taxes
run_computation_and_save("parameters_US_final_beta_inc_taxes.jl", "beta_inc_taxes") #US parameters with French beta, taxes, and income process 
...
plot(weights_beta, line = (3, :dot, [:red]), label = "Weights with French β")
plot!(weights_beta_taxes, line = (3, :dash, [:lightred]), label = "Weights with French β + Taxes")
plot!(weights_beta_inc_taxes, line = (3, :solid, [:blue]), label = "Weights with French β + Taxes + Income Process")
plot!(10 * disty, legend = :topleft, line = (2, :dash, [:darkgreen]), fill = (0, :lightgreen), fillalpha = 0.3, dpi = 300, label = "Distribution")
ylims!(0, 5)
savefig("Weights_comparison_total_US_2.png")
...
plot!(10 * disty, legend = :topleft, line = (2, :dash, [:darkgreen]), fill = (0, :lightgreen), fillalpha = 0.3, dpi = 300, label = "Distribution")
ylims!(0, 5)
savefig("Weights_comparison_total_US_param_2.png")
...
## Using the Parameters of France
include("parameters_France_final.jl")
κ1 = 0.974:0.0001:0.976
τ1 = τ:τ
#τ1 = 0.78:0.001:0.79
err_κ = 0.01
err_τ = 0.01
...
plot(weights,line = (3,:solid, [:blue]), label = "Weights")
plot!(weights_fr,line = (3,:dash, [:red]), label = "Weights with French system")
plot!(weights_marginal, line = (2,:solid, [:lightblue]), label = "Weights Marginal Utility")
plot!(weights_marginal_fr, line = (2,:dash, [:lightred]), label = "Weights Marginal Utility with French system")
plot!(10*disty,legend=:topleft, line = (2, :dash, [:darkgreen]),fill = (0, :lightgreen),fillalpha = 0.3,dpi=300, label = "Distribution")
ylims!(-0.2,5.0)
#savefig("Weights_us_france.png")
...
l = @layout [a ; b c;d]
p1 = plot(weights_fr .- weights, line = (2,:solid, [:blue]), label = "Δ Weights", ylims = (-0.10,0.15), legend=:topright )
p2 = plot(Utility_fr .- Utility, line = (2,:solid, [:blue]), label = "Δ Utility", ylims =  (-0.20,0.85), legend=:topright)
p3 = plot(Labor_Income_fr .- Labor_Income, line = (2,:solid, [:blue]), label = "Δ Labor Income", ylims = (-0.20,0.85), legend=:topright )
p4 = plot(Capital_Income_fr .- Capital_Income, line = (2,:solid, [:blue]), label = "Δ Capital Income", ylims = (-0.20,0.85) , legend=:topright )
plot(p1, p2, p3,p4,dpi = 300, title=["Weights " "Utility " "Labor Inc. " "Capital Inc."])
#savefig("Dif_Weights_us_france.png")


## Plotting the difference between the original calibration and the one using the French Tax System
l = @layout [a ; b c;d]
p1 = plot(weights_param_fr .- weights_param, line = (2,:solid, [:blue]), label = "Δ Weights", ylims = (-0.10,0.15), legend=:topright )
p2 = plot(Utility_fr .- Utility, line = (2,:solid, [:blue]), label = "Δ Utility", ylims = (-0.20,0.85), legend=:topright)
p3 = plot(Labor_Income_fr .- Labor_Income, line = (2,:solid, [:blue]), label = "Δ Labor Income", ylims = (-0.20,0.85), legend=:topright)
p4 = plot(Capital_Income_fr .- Capital_Income, line = (2,:solid, [:blue]), label = "Δ Capital Income", ylims = (-0.20,0.85), legend=:topright )
plot(p1, p2, p3,p4,dpi = 300, title=["Weights " "Utility " "Labor Inc. " "Capital Inc. "])
savefig("Dif_Weights_us_france_param.png")
...
wbar = 20.71 #average annual wages
...
zlims!(0.0,5.0)
xlabel!("Wealth in k of US\$")
ylabel!("Productivity level")
#savefig("weights_us_wealth.png")
...
zlims!(0.0,5.0)
xlabel!("Wealth in k of US\$")
ylabel!("Average wage per year in k of US\$ ")
#savefig("weights_us_wealth_income.png")
...
ylims!(0,4)
#savefig("weights_us_wealth_average.png")
...
#(weights_bin_y,legend=:topleft, line = (1,:solid, [:red]), label = "Weights for productivity", dpi=300)
ylims!(-0.2,5)
xlabel!("Histories")
ylabel!("Pareto weights")
savefig("weights_us_comparison.png")
...
plot!(10*disty,legend=:topleft, line = (2, :dash, [:darkgreen]),fill = (0, :lightgreen),fillalpha = 0.3,dpi = 300, label = "Distribution")
ylims!(0,5)
savefig("weights_us_comparison_history.png")